...
function load(){
  try{const d=JSON.parse(localStorage.getItem(KEY)||"null");
    if(d&&d.v===2&&d.skills){const f=fresh();SKILLS.forEach(s=>{const e=d.skills[s.id]; if(!e)d.skills[s.id]=f.skills[s.id]; else if(s.area==="prog"&&!e.pl){const pl=PLS[d.pl]?d.pl:"py";d.skills[s.id]={pl:{[pl]:{L:e.L,n:e.n,c:e.c}}}}}); // migra o domínio único da v3 para a linguagem em uso
      if(d.xpTotal===undefined)d.xpTotal=d.xp||0; for(const k in f)if(d[k]===undefined)d[k]=f[k]; d.sprint.b=d.sprint.b||{}; d.sprint.run=d.sprint.run||0; if(!MODES[d.mode])d.mode="normal"; if(!KT.MODELS[d.pilot])d.pilot="elo"; if(!d.runner)d.runner=f.runner; if(!d.sid)d.sid=newSid(); if(!d.lab)d.lab={}; if(!d.labLog)d.labLog=[]; if(!d.bonusDone)d.bonusDone={}; if(!d.session)d.session=f.session; if(!LANG[d.lang])d.lang="pt"; if(!PLS[d.pl])d.pl="py"; d.ai=d.ai||f.ai; d.brief=d.brief||{}; return d;}}catch(e){}
  return fresh();
}
...
function resolve(ok){
  ...
  S.log.push({i:S.log.length+1,sprint:S.sprint.n,item:it.id,skill:it.skill,bloom:it.bloom,d:it.d,ok,hint:cur.hint,ai:!!cur.aiUsed,mode:cur.mode,typed:!!cur.typed,timeout:!!cur.timeout,boss:it.boss||null,lang:S.lang,pl:isProg(it.skill)?S.pl:null,before:+before.toFixed(3),after:+sk.L.toFixed(3),v:2,pilot:pilot(),preds,pct:pct==null?null:+pct.toFixed(3),code:it.type==="code"?{hash:cur.hash||null,chars:(cur.code||"").length,lang:cur.lang||S.pl,modo:cur.res&&cur.res.modo,tel:cur.tel,extra:!!it.extra,kind:it.kind||null,bonus:!!cur.bonus,regras:(cur.res&&cur.res.regras||[]).map(x=>[x.id,x.ok])}:null,b:+KT.itemB(it).toFixed(2),c:+g.toFixed(3),session:ses.id,pos:ses.n,rt:cur.t0?now-cur.t0:null,t:now});
...
function exportData(){return JSON.stringify({app:"DevWise",version:4,student:S.sid,skills:S.skills,log:S.log},null,1)}
/* Tabela longa para pesquisa: uma linha por resposta. pred_* = previsão gravada no jogo (só em registros v2);
   rpred_* = previsão reconstruída por replay, a partir do mesmo prior neutro para todos os modelos. */
const RESEARCH_COLS=["schema","student","row","timestamp","session","position","sprint","item","skill","area","pl","item_type","bloom","difficulty","b","c","correct","score","mode","hint","typed","ai","bonus","boss","pilot","log_version","pred_elo","pred_irt","pred_bkt","pred_pfa","pred_afm","rpred_elo","rpred_irt","rpred_bkt","rpred_pfa","rpred_afm"];
function researchRows(){
  const rows=S.log.filter(l=>IT[l.item]);
  let ses=0,last=0,pos=0,lastPl=null;   /* linguagem ausente em linha antiga: usa a última registrada, que era a do jogo naquele momento */
  const base=rows.map(l=>{const it=IT[l.item],prog=isProg(l.skill);
    if(l.session!=null){ses=l.session;pos=l.pos}else{if(!last||l.t-last>30*60000){ses++;pos=0}pos++}   /* sessão derivada nas linhas antigas: mesma regra do jogo */
    last=l.t;
    let c=l.c!=null?l.c:guessProb(it); if(l.c==null){if(l.typed)c=0.03; else if(l.hint)c=Math.max(.5,c)}
    const pl=prog?(l.pl||lastPl||S.pl):""; if(prog&&l.pl)lastPl=l.pl;
    return {l,it,ses,pos,c,pl,key:l.skill+"|"+(prog?pl:"-"),y:l.ok?1:0};});
  const rp=KT.replay(base.map(x=>({key:x.key,item:x.it,c:x.c,y:x.y})),L0);
  const f=v=>v==null?"":typeof v==="number"?String(+v.toFixed(4)):String(v);
  return base.map((x,i)=>{const l=x.l,p=l.preds||{},r=rp[i];
    return ["devwise-kt-1",S.sid,i+1,new Date(l.t).toISOString(),x.ses,x.pos,l.sprint,l.item,l.skill,SK[l.skill].area,x.pl,x.it.type,l.bloom,l.d,
      KT.itemB(x.it),x.c,x.y,l.pct==null?"":l.pct,l.mode,l.hint?1:0,l.typed?1:0,l.ai?1:0,l.code&&l.code.bonus?1:0,l.boss||"",l.pilot||"",l.v||1,
      p.elo,p.irt,p.bkt,p.pfa,p.afm,r.elo,r.irt,r.bkt,r.pfa,r.afm].map(f);});
}
function exportCSV(){const q=v=>/[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v;return [RESEARCH_COLS.join(","),...researchRows().map(r=>r.map(q).join(","))].join("\n")}
function report(){
    ...
    h("div",{class:"grid2"},
      ...
      h("section",{class:"panel noprint"},h("h3",null,t("data")),h("p",{class:"note"},t("dataP")),
        h("div",{class:"row"},
          ...
          h("button",{class:"btn ghost small",onclick:()=>{ if(!resetArmed){resetArmed=true;render();return;} const keep={sid:S.sid,lang:S.lang,pl:S.pl,ai:S.ai,mode:S.mode,pilot:S.pilot,confirm:S.confirm,name:S.name,runner:S.runner}; S=Object.assign(fresh(),keep);save();selSkill=null;cur=null;go("home") }},resetArmed?t("resetConfirm"):t("reset"))),
        h("button",{class:"btn ghost small",onclick:()=>{try{const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([exportCSV()],{type:"text/csv"}));a.download="devwise-"+S.sid+".csv";a.click()}catch(e){}}},"📊 "+t("csvBtn")),
        h("p",{class:"note"},t("csvNote",{s:S.sid})),
...
function settings(){
  ...
  return h("main",{class:"work"},h("h2",null,t("setH")),
    ...
    open&&h("section",{class:"teacher"},
      ...
      h("p",null,h("b",null,t("sidLabel")+" "),h("code",null,S.sid)),
      h("h3",null,t("modelH")),
